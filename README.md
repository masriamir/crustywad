...
```toml
[dependencies]
crustywad = { version = "0.4.0", features = ["write", "mmap"] }
```
...
`REJECT` and `BLOCKMAP` decode into typed, queryable structures (`MapReject` sector-visibility lookups, `MapBlockmap` per-block linedef lists) during map assembly, and Doom 64's `LEAFS` render leaves decode into a per-subsector `MapLeaf` arena, and `MACROS` scripts decode read-only into `MapMacro` action lists.
