...
```toml
[dependencies]
crustywad = { version = "0.6.0", features = ["write", "mmap"] }
```
...
| `nodebuild` | no | Enables the `map::build` node-lump builders (implies `write`) — `build_reject` and `MapReject::to_lump_bytes` for clean-room REJECT generation (ADR-0024). |
