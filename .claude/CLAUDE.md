...
| `nodebuild` | no | Enables the `map::build` node-lump builders (implies `write`) — `build_reject` and `MapReject::to_lump_bytes` for clean-room REJECT generation (ADR-0024); BLOCKMAP/BSP builders arrive in later stages |
