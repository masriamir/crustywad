...
7. Wait at least 5 minutes, then confirm via `gh api graphql` with a query of the form `{ repository(owner: ..., name: ...) { pullRequest(number: ...) { reviews(last: 20) { nodes { author { login } submittedAt } } } } }` that a review with `author.login` equal to `copilot-pull-request-reviewer` exists (GraphQL omits the `[bot]` suffix used when requesting the reviewer via REST) with `submittedAt` after the request time. Use a generous `last` count (20, not 5) — a small window can push the target review out if several other reviews land afterward. Elapsed wall-clock time alone is not a reliable completion signal — if no such review is found yet, keep waiting rather than checking `reviewThreads`.
8. Once that review is found, wait one further ~5 minutes and re-fetch `reviewThreads` again. Copilot has been observed to post additional comments in a second wave up to ~5 minutes after its review event first appears (e.g. PR #136: a round was marked resolved at 15:30, but two more legitimate comments landed at 15:35). Treat the round as complete only if this second check also shows zero new unresolved threads.
