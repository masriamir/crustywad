...
6. Once every thread from the current round is resolved, first fetch a baseline using the `reviews` query from step 7: from its `nodes`, filter to entries where `author.login` is `copilot-pull-request-reviewer` and take the one with the maximum `submittedAt` (or note that no such entry exists yet) — the list is not necessarily all-Copilot, so do not assume the last node overall is the right one. Then request a fresh Copilot review (`POST /pulls/{number}/requested_reviewers` with `copilot-pull-request-reviewer[bot]`). Use this GitHub-recorded baseline rather than a locally captured wall-clock timestamp — comparing a local timestamp to GitHub's `submittedAt` mixes two clock domains and is vulnerable to clock skew and to an unrelated review landing in the gap between capturing the timestamp and issuing the request.
7. Poll every ~60 seconds — do not wait a flat 5 minutes here — confirming via `gh api graphql` with a query of the form `{ repository(owner: "OWNER", name: "REPO") { pullRequest(number: 140) { reviews(last: 20) { nodes { author { login } submittedAt } } } } }` whether a review with `author.login` equal to `copilot-pull-request-reviewer` exists (GraphQL omits the `[bot]` suffix used when requesting the reviewer via REST) with `submittedAt` strictly after the step 6 baseline — comparing two GitHub-recorded timestamps this way avoids the clock-skew problem a local timestamp would introduce. If step 6 found no prior Copilot review (no baseline), any `copilot-pull-request-reviewer` review found here counts as a match — there is nothing to compare against. Use a generous `last` count (20, not 5) — a small window can push the target review out if several other reviews land afterward. In practice the review has landed within 1-2 minutes each time observed, so 60s polling finds it far sooner than a fixed 5-minute wait would — if no such review is found yet, keep polling rather than checking `reviewThreads`. If no review appears after ~15 minutes of polling (well beyond the 1-2 minute norm), stop: don't keep polling indefinitely. Treat this as a possible bot/service failure, note it, and surface the stall to a human rather than continuing the loop.
8. Once that review is found, wait ~5 minutes from the moment it was found (this dwell is deliberate and is not shortened by the tighter polling in step 7 — do not fetch `reviewThreads` any earlier), then fetch `reviewThreads` (the query from step 1) as the first of two required thread checks. This is not a query-visibility delay — in every instance measured so far across multiple PRs, a comment's `createdAt` has matched its review's `submittedAt` to within a second (an observed pattern, not a documented GitHub guarantee), so comments have not been seen arriving late relative to their own review. The risk this dwell guards against is a distinct, later Copilot review landing without an explicit re-request (e.g. PR #136: a round was marked resolved at 15:30, then a further review with its own legitimate comments landed at 15:35) — step 7 only detects the review it polled for, not one that shows up afterward on its own, so checking `reviewThreads` too soon would miss that second review's comments entirely.
9. If step 8 is clean, wait a further ~5 minutes and fetch `reviewThreads` again as the second required thread check.
