...
- Strictness: strict rejects a lump whose length matches neither modulus
  ...
  about trailing bytes. Contract shape (as finalized in #393):
  `Result<(Vec<DialogueRecord>, DialogueFormat, Vec<DialogueWarning>),
  DialogueError>` — the selected layout and the warnings are both observable
  on success.
