...
| `parse/from_path` | `Wad::from_path` and `from_path_with_options` on a tempfile-backed medium WAD; mmap variants when the `mmap` feature is enabled |
...
| `write/build_lenient` | `WadBuilder::build_with_options(lenient)` on the same sizes |
...
Benchmarks are part of the standard workspace — no extra tooling required beyond a stable
Rust toolchain.
