...
/// all are valid ASCII and accepted by `WadBuilder::add_lump` without error.
/// Uppercase-only would also be valid, but narrowing the charset would underspecify
/// what the write path actually accepts.
#[allow(dead_code)]
