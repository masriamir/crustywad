...
| `extended-nodes-zlib` | no | Enables reading the zlib-compressed ZDoom extended node formats (`ZNOD`/`ZGLN`/`ZGL2`/`ZGL3`) by inflating (pure-Rust `miniz_oxide`) to their uncompressed twins, bounded by `Limits::max_decoded_node_bytes`; off by default so the core build stays decompressor-free (ADR-0025 §5). With `nodebuild` also enabled, also powers the `nodebuild` `ZNOD` and `Z*` GL writers |
